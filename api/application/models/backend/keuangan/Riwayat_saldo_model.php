...
class Riwayat_saldo_model extends CI_model
{
    public $table = 'history_saldo a'; // you MUST mention the table name
    ...
    public function get_all_count($where,$search){
      $this->db->select('a.id')
       ...
       if(!empty($search)){
         $this->db->where("(b.username LIKE '%$search')");
       }
    ...
    public function get_all($where,$search,$order,$limit,$offset){
      $this->db->select('a.*,b.username as username_pengguna')
               ->from($this->table)
               ->join('pengguna b','a.id_pengguna = b.id')
               ->where('a.deleted',1)

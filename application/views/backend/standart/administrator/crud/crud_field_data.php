 ...
 <table class="table table-responsive table table-bordered table-striped"  id="diagnosis_list">
   ...
   <tbody>
      ...
      <tr>
		 ...
		 <?php 
			$mycol = $row->name;
			if(substr($row->name,0,3) == "id_" && !$row->primary_key){
				$mycol = substr($row->name,3);
			}
		 ?>
         <input type="text" class="crud-input-initial" name="crud[<?=$i; ?>][<?=$row->name; ?>][label]" placeholder="<?= $row->name; ?>" value="<?= $mycol; ?>">
               
         </td>
         <td class="column">
            <input class="flat-red check" type="checkbox" <?= ($row->primary_key || $row->name == 'deleted'  || $row->name == 'date_add'  || $row->name == 'add_by')   ? '' : 'checked'; ?> name="crud[<?=$i; ?>][<?=$row->name; ?>][show_in_column]" value="yes">
         </td>
         <td class="add_form">
            <input class="flat-red check" type="checkbox" <?= ($row->primary_key || $row->name == 'deleted'  || $row->name == 'date_add'  || $row->name == 'add_by') ? '' : 'checked'; ?> name="crud[<?=$i; ?>][<?=$row->name; ?>][show_in_add_form]" value="yes">
         </td>
         <td class="update_form">
            <input class="flat-red check" type="checkbox" <?= ($row->primary_key || $row->name == 'deleted'  || $row->name == 'date_add'  || $row->name == 'add_by') ? '' : 'checked'; ?> name="crud[<?=$i; ?>][<?=$row->name; ?>][show_in_update_form]" value="yes">
         </td>
         <td class="detail_page">
            <input class="flat-red check" type="checkbox" <?= ($row->primary_key || $row->name == 'deleted'  || $row->name == 'date_add'  || $row->name == 'add_by') ? '' : 'checked'; ?> name="crud[<?=$i; ?>][<?=$row->name; ?>][show_in_detail_page]" value="yes">
         </td>
         <td>
            <div class="col-md-12">
